...
// The generic (non-Amiga specific) parts of src/Change.c
...
// FIXME: Remove duplication w/Amiga specific version
 ...
 ********/
int Change(BufStruct *Storage, int type, int vald, int newvalue)
{
  int ret=0;
  WORD settingtype;
  BOOL changed=FALSE;
...
int ChangeAsk(BufStruct *Storage, int vald, int *input)
{
  int temp;
  int ret=0;
...
int ReDrawSetting(BufStruct *Storage, WORD update)
{
...
static int bsearch_cmp_gsn(void *name, int count)
{
  return(strcmp(sets[count]->name, (char *)name));
}
int Bsearch(void *name, int antal, int (cmp)(void *, register int))
{
  int vald=-1;
